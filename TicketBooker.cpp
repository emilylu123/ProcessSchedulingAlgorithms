...
Customer initialCustomer(string aLine)
{
    ...
    Customer aCustomer;
    aCustomer.processId = info [0];
    aCustomer.arrivalTime = info [1];
    aCustomer.priority = info [2];
    aCustomer.age = info [3];
    aCustomer.ticketsRequire = info [4];
    aCustomer.isReady = false;
    aCustomer.hasDone = false;
...
bool cmpArrivalTime(const Customer& a, const Customer& b) {
...
bool cmpPriority(const Customer& a, const Customer& b) {
    return a.priority < b.priority;
}

// the condition of end time.
bool cmpRemainingTime(const Customer& a, const Customer& b) {
...
void demoteQueue(vector<Customer>& queue1, vector<Customer>& queue2){
    ...
    sort(queue2.begin(),queue2.end(),cmpPriority);
    sort(queue2.begin(),queue2.end(),cmpRemainingTime);
}
...
bool waitingAgingQ2(vector<Customer>& queue1, vector<Customer>& queue2, int& ticket_dealing_time){
    ...
    {
        if (queue2[i].isReady)
            queue2[i].waitingTime += ticket_dealing_time;
...
//    sort(queue1.begin(),queue1.end(),cmpID);
    sort(queue1.begin(),queue1.end(),cmpPriority);
...
int main(int argc,char *argv[]){
    ...
    sort(inputs.begin(),inputs.end(),cmpArrivalTime);
